...
if ( ! class_exists( 'WC_Shipping_Distance_Rate' ) ) {
	...
	class WC_Shipping_Distance_Rate extends WC_Shipping_Method {
		...
		public function init_form_fields() {
			$this->form_fields  = array(
				...
				'address_1'             => array(
					...
					'type'            => 'text',
					'description'     => __( 'First address line of the company where you are shipping from.', 'wc-distance-rate' ),
				),
				'address_2'             => array(
					'title'           => __( 'Address 2', 'wc-distance-rate' ),
					'type'            => 'text',
					'description'     => __( 'Second address line of the company where you are shipping from.', 'wc-distance-rate' ),
				),
				'city'             => array(
					'title'           => __( 'City', 'wc-distance-rate' ),
					'type'            => 'text',
					'description'     => __( 'City of your company where you are shipping from.', 'wc-distance-rate' ),
				),
				'postal_code'             => array(
					'title'           => __( 'Zip/Postal Code', 'wc-distance-rate' ),
					'type'            => 'text',
					'description'     => __( 'Zip or Postal Code of your company where you are shipping from.', 'wc-distance-rate' ),
				),
				'state_country'             => array(
					'title'           => __( 'State/Provice/Country', 'wc-distance-rate' ),
					'type'            => 'text',
					'description'     => __( 'State/Provice or Country of your company where you are shipping from.', 'wc-distance-rate' ),
				),
		...
		public function calculate_shipping( $package = array() ) {
			...
			if ( isset( $distance->rows[0]->elements[0]->status ) && 'ZERO_RESULTS' ==  $distance->rows[0]->elements[0]->status ) {
				return;
			}
			...
			if ( 'imperial' == $this->unit ) {
				$distance = round( $distance->rows[0]->elements[0]->distance->value * 0.000621371192 );
			} else {
				$distance = round( $distance->rows[0]->elements[0]->distance->value / 1000 );
			}
		...
		public function get_shipping_address_string() {
			...
			if ( isset( $this->state_country ) && ! empty( $this->state_country ) ) {
				$address[] = $this->state_country;
			}
			return implode( ', ', $address );
