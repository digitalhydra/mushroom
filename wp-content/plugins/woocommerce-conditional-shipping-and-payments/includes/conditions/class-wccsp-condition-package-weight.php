...
class WC_CSP_Condition_Package_Weight extends WC_CSP_Condition {
	...
	public function __construct() {
		...
		$this->title                         = __( 'Package Weight', WC_Conditional_Shipping_Payments::TEXT_DOMAIN );
	...
	public function get_condition_resolution( $data, $args ) {
		...
		if ( $data[ 'modifier' ] === 'min' && $data[ 'value' ] <= $pkg_weight ) {
			return sprintf( __( 'decrease the package weight below %1$s%2$s', WC_Conditional_Shipping_Payments::TEXT_DOMAIN ), $data[ 'value' ], get_option( 'woocommerce_weight_unit' ) );
		}

		if ( $data[ 'modifier' ] === 'max' && $data[ 'value' ] > $pkg_weight ) {
			return sprintf( __( 'increase the package weight above %1$s%2$s', WC_Conditional_Shipping_Payments::TEXT_DOMAIN ), $data[ 'value' ], get_option( 'woocommerce_weight_unit' ) );
		}
